...
function create_results(){
    ...
    var fake_res_1 = result + z2_n;
    var fake_res_2 = result - z2_n;
    var fake_res_3;

    if(fake_res_2 < 0){
        fake_res_2 = fake_res_2 * (-1);
    }


    if(Math.random() <  0.5){
        fake_res_3 = result + Math.floor(Math.random() * (z2_n - 1 - 1 + 1)) + 1;
    }else{
        fake_res_3 = result - Math.floor(Math.random() * (z2_n - 1 - 1 + 1)) + 1;
        if(fake_res_3 < 0){
            fake_res_3 = fake_res_3 * (-1);
        }
    }
